...
class SSerializer {
  ...
  static Future<Map<String, String>> buildFile(HttpRequest request,
      {required String pathToSaveFile,
      String? responseOnSaved,
      String? responseOnUnsaved}) async {
    if (request.headers.contentType?.mimeType == 'multipart/form-data') {
      ...
      await for (var part in parts) {
        ...
        final filename =
            RegExp(r'filename="(.*)"').firstMatch(contentDisposition!)![1];
